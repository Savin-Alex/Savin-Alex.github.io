...
Production-like прототип (2026): процесс из трёх AI-агентов, который проводит небольшую задачу от Jira до готового Pull Request — с участием человека, проверками качества и трассируемостью решений.
...
<iframe src="{{ '/demos/ai-dev-pipeline.ru.html' | relative_url }}" title="Интерактивное демо: Multi-Agent AI Development Pipeline" loading="lazy" style="width:100%;height:580px;border:1px solid #e2e6ee;border-radius:8px;"></iframe>
...
| Поле | Содержание |
|---|---|
| **Проблема** | AI-coding демо — одношаговый сценарий (вставил задачу → получил код → проверил вручную). Это помогает продуктивности, но это не процесс поставки: нет intake, декомпозиции, проверки, ревью, трассируемости и ответственности человека. |
| **Цель** | Показать, как GenAI встраивается в delivery не как «автогенерация кода», а как контролируемый workflow с acceptance criteria, quality gates, risk management, метриками и evidence package. |
| **В scope** | Jira issue intake → planning → implementation → sandbox validation → reviewer approval → GitHub PR; run evidence; rollback plan; production-like governance через GitHub Actions. |
| **Вне scope** | Параллельное исполнение; доп. агенты (DevOps / документация); production-контейнеризация; автоматизация больших расплывчатых задач. |

**Критерии успеха (со статусом):**
- AI-вывод недоверенный до явных проверок (тесты, sandbox, secret scan, ревью) → **выполнено**
- dry-run по умолчанию, real-run только контролируемо (GitHub Actions) → **выполнено**
- Локальный контур стабилен → **выполнено** (55 тестов, lint чистый)
- Evidence по каждому прогону (summary, метрики, логи) → **выполнено** (150 run summaries по 11 issue-группам)
...
Процесс разделён на безопасный dry-run и контролируемый реальный прогон — чтобы было видно, что агенты не пушат код сами по себе, а работают внутри управляемого контура.

- Приём Jira-задачи с критериями приёмки.
- Планирование и предложение по реализации.
- Проверка в ограниченном sandbox-окружении (без доступа к сети).
...
Вывод AI считается недоверенным, пока не пройдёт явные проверки:
...
- **Сработало:** связал Jira → AI-этапы → sandbox validation → review → GitHub PR в единый delivery flow; quality gates делают процесс управляемым; run evidence объясняет результат каждого прогона; документация покрывает scope, risks, metrics, rollback и acceptance.
- **Улучшить дальше:** dashboard по метрикам; approval matrix для ролей PM / Tech Lead / Security / Operator; incident report template; больше benchmark runs; публичный sanitized evidence bundle.
- **Ключевой вывод:** ценность PM здесь не в том, чтобы «сделать AI-пайплайн», а в том, чтобы превратить технический эксперимент в понятный, измеримый и управляемый delivery process.
