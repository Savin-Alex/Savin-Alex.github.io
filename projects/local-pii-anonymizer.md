...
[RU](./local-pii-anonymizer.html) · [EN](../en/projects/local-pii-anonymizer.html)
...
**Роль:** IT Project Manager · соло-портфолио-проект; на мне постановка задачи, архитектура, разработка и контроль качества.
...
## Следующий этап: privacy-шлюз с API (август 2026)

Тот же движок детекции стал ядром privacy-шлюза — веб-API, к которому могут обращаться другие системы. Полная история исходного инструмента сохранена. У каждого этапа были письменные критерии приёмки, и этап проходил их до старта следующего.

| Этап | Статус | Результат |
|---|---|---|
| Выделить движок | Готово | Код детекции, псевдонимизации и хранилища стал отдельным пакетом, который может использовать любое приложение |
| Версионированный веб-API | Готово, принято после независимого ревью | Шлюз `/api/v1` с проверкой личности, база данных только с метаданными, зашифрованные маппинги с коротким сроком жизни и сгенерированный клиентский контракт |
| Веб-приложение | Не начато | Браузерный интерфейс поверх API |

Критерии приёмки — это правила приватности, а не список функций: исходный, защищённый и восстановленный текст, имена файлов, маппинги и ключи никогда не попадают в базу данных, логи и сообщения об ошибках; запрос на удаление фиксируется до стирания данных; токены личности истекают и не могут быть использованы повторно. Все находки ревью закрыты до приёмки этапа API.

За этот же период движок получил распознаватели Emirates ID (ОАЭ) и ID резидента Китая, а исправление перестало давать номеру карты скрывать национальные ID с проверенной контрольной суммой. Потребительский продукт на этом движке — отдельный кейс: [RemovePII](./removepii.html).
...
**Next steps:** веб-приложение поверх принятого API шлюза; Windows CI + smoke-тест; production deployment profile (TLS, secret store, encrypted storage, audit sink, dependency lock); privacy review packet (data inventory, DPIA, retention/deletion policy); pilot go/no-go review (риски, evidence, rollback).
...
Python 3.12, FastAPI, PostgreSQL, Redis, OpenAPI, Streamlit, Pydantic, structlog, Docker, Presidio, spaCy, HuggingFace Transformers, Ollama, Hypothesis, pytest, mypy, ruff, bandit.
