...
[RU](../../projects/ai-consul.html) · [EN](./ai-consul.html)
...
**Role:** IT Project Manager · solo portfolio project; product, UX, engineering, and delivery controls on me.
...
## Reliability from real session logs (Aug 2026)

Each real session produces a numbered log. In August I ran a log-driven cycle (Logs29–Logs39): every failure class found in a log became a typed control and a test, not a one-off patch.

| Log finding | Control added |
|---|---|
| A "silent hour": a retired model returned 404, then a CDN edge returned 403, and the session produced no answers | Circuit breakers typed by failure class (retired model, forbidden, auth, request rejected); a breaker on one model no longer blocks sibling models |
| A session with zero usable answers from a reasoning model | Model-aware payloads and an explicit reasoning/output budget |
| Cost and retries were invisible when every provider failed | A per-attempt LLM ledger that also records 429s and survives total failure |
| Latency was a feeling, not a number | Real per-request LLM latency telemetry; the reasoning-control fix measured at 2.5–3.3 s; a paid fallback route answered 4/4 times under 8 s |
| Mixed-speaker utterances and stale transcript replays corrupted rows | Word-level speaker runs at cloud seams; 11 review rounds closed the stale-replay paths |
| The docs claimed more pause handling than was proven | A scripted pause test passed, and the claim was narrowed to what the test showed |
