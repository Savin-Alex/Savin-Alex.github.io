...
[RU](../../projects/local-pii-anonymizer.html) · [EN](./local-pii-anonymizer.html)
...
**Role:** IT Project Manager · solo portfolio project; problem framing, architecture, development, and quality on me.
...
## Next stage: a privacy gateway API (Aug 2026)

The same detection engine became the core of a privacy gateway: a web API that other systems can call. The full history of the original tool is kept. Each stage had written acceptance criteria and passed them before the next stage started.

| Stage | Status | Result |
|---|---|---|
| Separate the engine | Done | The detection, pseudonymization, and vault code became a stand-alone package that any app can reuse |
| Versioned web API | Done, accepted after an independent review | `/api/v1` gateway with verified identity, a database that stores metadata only, encrypted short-lived mappings, and a generated client contract |
| Web application | Not started | Browser interface on top of the API |

The acceptance criteria were privacy rules, not a feature list: original, protected, and restored text, file names, mappings, and keys never reach the database, logs, or error messages; a delete request is recorded before the data is erased; identity tokens expire and cannot be replayed. All review findings were closed before the API stage was accepted.

In the same period, the engine gained UAE Emirates ID and China resident ID recognizers, and a fix stopped card-number detection from hiding checksum-validated national IDs. The consumer product built on this engine is a separate case: [RemovePII](./removepii.html).
...
**Next steps:** a web application on top of the accepted gateway API; Windows CI + smoke test; production deployment profile (TLS, secret store, encrypted storage, audit sink, dependency lock); privacy review packet (data inventory, DPIA, retention/deletion policy); pilot go/no-go review (risks, evidence, rollback).
